...
const OrderController = {
  ...
  update: async (req, res) => {
    ...
    try {
      ...
      //   });
      // }
      const checkReceived = updatedOrder.statusHistory.some(
        (item) => item.status === 'received',
      );
      const checkCancel = updatedOrder?.statusHistory?.some(
        (item) => item.status === 'cancelled',
      );
      if (
        value.returnInfo?.status == 'returned' &&
        value.returnInfo?.items?.length > 0
      ) {
        const totalPrice = value.returnInfo?.items.reduce((total, item) => {
      ...
      if (value.status === 'cancelled' && !checkCancel) {
      ...
      if (updatedOrder.status === 'received' && !checkReceived) {
